...
protocol ICardLayout {
    ...
    func pointForInitState(for column: Int, row: Int) -> CGPoint
...
final class CardLayout: ICardLayout {
    ...
    func point(for column: Int, row: Int) -> CGPoint {
        if column < 7 {
            return CGPoint(
                x: columns[column].x,
                y: columns[column].y + CGFloat(row) * offsetY
            )
        } else if column == 7 {
            return extra
        } else if column == 8 {
    ...
    func pointForInitState(for column: Int, row: Int) -> CGPoint {
        if column <= Int.tStacksMaxInd {
            return CGPoint(
                x: columns[column].x,
                y: columns[column].y + offsetY / 2 * CGFloat(row)
            )
        } else {
            return point(for: column, row: row)
        }
    }
